...
class ValoEnhancedElite:
    ...
    def _step(self):
        ...
        # --- Duelists guided by elites ---
        for d in duelists_idx:
            if self.archive and self.rng.rand() < 0.4:
                elite = self.archive[self.rng.randint(len(self.archive))]
                target = elite
            else:
        ...
        # --- Controllers move toward elite centroid ---
        for c in controllers_idx:
            ...
            if len(active_idx) > 0:
                ...
                if self.archive:
                    elite_centroid = np.mean(np.array(self.archive), axis=0)
                    centroid = 0.6 * elite_centroid + 0.4 * centroid
        ...
        # --- Sentinels protect and exploit elites ---
        for s in sentinels_idx:
            if self.best is not None:
                step = self.p["lmbda"] * (self.best - self.positions[s])
                if controller_centroid is not None:
                    step += 0.2 * (controller_centroid - self.positions[s])
                if self.archive and self.rng.rand() < 0.3:
                    elite = self.archive[self.rng.randint(len(self.archive))]
                    step += 0.3 * (elite - self.positions[s])
        ...
        if self.archive:
            all_elites = np.vstack([self.archive, elites])
        else:
            all_elites = np.array(elites)

        unique_elites = []
        for e in all_elites:
            if not any(np.linalg.norm(e - u) < 1e-6 for u in unique_elites):
                unique_elites.append(e)
        elite_scores = [self.func(e) for e in unique_elites]
        sorted_elites = [e for _, e in sorted(zip(elite_scores, unique_elites), key=lambda z: z[0])]
        self.archive = sorted_elites[:5]


        if self.rng.rand() < 0.1:
            worst_idx = np.argmax(self.fitness)
            elite = self.archive[self.rng.randint(len(self.archive))]
            mutated = self._clip(elite + 0.05 * self.rng.randn(self.dim))
            self.positions[worst_idx] = mutated
            self.fitness[worst_idx] = self.func(mutated)
...
opt = ValoEnhancedElite(rastrigin, lb, ub, 100, n_agents=600, seed=42)
